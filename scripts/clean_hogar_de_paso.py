...
def clean_hogar_partitions():
    print(f"🧹 Iniciando limpieza de particiones antiguas para 'hogar_de_paso'...")
    
    client = storage.Client(project=PROJECT_ID)
    bucket = client.bucket(RAW_BUCKET)
    
    # Borrar todo lo que sea partición diaria (y=...)
    # Dejaremos intacta la carpeta 'latest' (que es la correcta)
    prefix = "supabase/hogar_de_paso/y="
